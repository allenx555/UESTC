...
6.GetWord              // get a 32bit word ki from bit strings k[i],k[i+1]..., // namely ki=k[i]||k[i+1]||…||k[i+31] 7.LFSRWithInitMode     // Initialisation mode,refresh the current state of LFSR 
...
/************************************************************ 
Function:     GetWord 
Description:  get a 32bit word ki from bit strings k[i],k[i+1]...,namely ki=k[i]||k[i+1]||…||k[i+31] Calls: 
Called By:    ZUC_Integrity 
Input:        k[]: 
              i: the position i 
Output: Return:       ki=k[i]||k[i+1]||…||k[i+31] 
Others: 
************************************************************/
...
Others: 
************************************************************/
void LFSRWithInitMode(unsigned int LFSR_S[], unsigned int u)//LFSR初始化模式
{
...
Others: 
************************************************************/
void LFSRWithWorkMode(unsigned int LFSR_S[])//LFSR工作模式
{
    unsigned int v = LFSR_S[0], i;
    v = AddMod(v, PowMod(LFSR_S[15], 15));
    v = AddMod(v, PowMod(LFSR_S[13], 17));
...
Others: 
************************************************************/
void BR(unsigned int LFSR_S[], unsigned int BR_X[])//比特重组
{
...
Others: 
************************************************************/
unsigned int F(unsigned int BR_X[], unsigned int F_R[])//非线性函数F
{
...
Others: 
************************************************************/
void ZUC_Init(unsigned char k[], unsigned char iv[], unsigned int LFSR_S[], unsigned int BR_X[], unsigned int F_R[])//祖冲之密码的运行：初始化阶段
{
...
Others: 
************************************************************/
void ZUC_Work(unsigned int LFSR_S[], unsigned int BR_X[], unsigned int F_R[], unsigned int pKeyStream[], int KeyStreamLen)//祖冲之算法的运行：工作阶段
{
    int i = 0;
    BR(LFSR_S, BR_X);
    F(BR_X, F_R);
...
Others: 
****************************************************************/
void ZUC_GenKeyStream(unsigned char k[], unsigned char iv[], unsigned int KeyStream[], int KeyStreamLen)//祖冲之算法的运行
{
...
iv[16]={0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff}; 
    ...
    //1
    unsigned char IK[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
